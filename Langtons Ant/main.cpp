...
void ant_thread(uint64_t start, uint64_t end) {
    for (uint64_t i = start; i < end; i++) {
        ...
        for (uint64_t j = 0; j < MAX_ITERATIONS; j += 4) { // this loop has been optimized to be as fast as possible on my hardware and environment, you may want to play around with it to find something that works better for you.
            uint8_t _grid = grid[index];
            uint8_t next = _grid < size_minus_one ? _grid + 1 : 0;
            grid[index] = next;
            if (pattern[_grid]) ant_direction = -ant_direction;
            index += ant_direction;
            ant_position_x += ant_direction;
            if (ant_position_x >= GRID_SIZE) break;
            _grid = grid[index];
            next = _grid < size_minus_one ? _grid + 1 : 0;
            grid[index] = next;
            if (!pattern[_grid]) ant_direction = -ant_direction;
